# -*- coding: utf-8 -*-
# Fused recurrent OSGM with per-head learnable decay on preconditioner D.
# Used for inference (seq_len <= 64) and as backward pass reference.
...
def fused_recurrent_osgm_decay_fwd_kernel(
    q, k, v, u, d_out, beta, gamma, o,
    ...
    # Load per-head decay
    b_gamma = tl.sigmoid(tl.load(gamma + i_h).to(tl.float32))
...
def fused_recurrent_osgm_decay_fwd(
    q, k, v, beta, gamma_log, scale, eta,
    ...
    grid = (NV, NK, N * H)
    fused_recurrent_osgm_decay_fwd_kernel[grid](
        q, k, v, u, d_out, beta, gamma_log, o,
...
class FusedRecurrentDecayFunction(torch.autograd.Function):

    @staticmethod
    @input_guard
    def forward(
        ctx, q, k, v, beta, gamma_log, scale, eta,
        ...
        o, u, d_out, final_state, final_scale = fused_recurrent_osgm_decay_fwd(
            q=q, k=k, v=v, beta=beta, gamma_log=gamma_log,
            scale=scale, eta=eta,
        ...
        # For simplicity, backward uses the chunk kernel's backward pass.
        # This forward-only kernel is mainly used for inference (no backward needed).
        ctx.mark_non_differentiable(o)
        return o, final_state, final_scale


@torch.compiler.disable
def fused_recurrent_delta_rule_osgm_decay(
    q, k, v, beta=None, gamma_log=None,
    scale=None, eta=None,
    ...
    o, final_state, final_scale = FusedRecurrentDecayFunction.apply(
        q, k, v, beta, gamma_log, scale, eta,
