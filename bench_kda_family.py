"""Benchmark KDA family (KDA, OS-KDA, OS-KDA-APF) on gpu-6 with gpu-6 fla code.

Same protocol as the main gpu-7 unified benchmark, just for the KDA family
where the model code on gpu-6 matches the trained checkpoints.
"""
from __future__ import annotations
import argparse, csv, statistics, time, os
...
def main():
    ...
    ap.add_argument("--output", default="/tmp/bench_kda_family.csv")
    args = ap.parse_args()

    device = torch.device("cuda")
    root = Path(os.environ.get("OSLA_ROOT", str(Path(__file__).resolve().parent)))
    EXP = root / "experiments/osla_340M/exp"

    variants = [
        ("KDA", "KDA", str(EXP / "kda-340M-baseline")),
        ("OS-KDA", "KDA", str(EXP / "os-kda-340M-detach-phase1-no-dd-eta0p01-dmax1p2-bwdpatch2-full-20260505-185514")),
        ("OS-KDA-APF", "KDA", str(EXP / "os-kda-340M-dd-eta0p003-dmin0p667-dmax1p5-4gpu-full-20260506")),
    ]

    results = []
    for label, fam, path in variants:
    ...
    print("\n=== KDA-family on gpu-6 GPU 4 ===")
    print("| Variant | tokens/sec | prefill ms | decode ms | KV MiB bf16 |")
    print("|---|---:|---:|---:|---:|")
    for r in results:
        print(f"| {r.label} | {r.tokens_sec:.1f} | {r.prefill_ms:.1f} "
              f"| {r.decode_ms:.1f} | {r.kv_mib_bf16:.3f} |")
