#!/usr/bin/env python3
"""FineWeb-Edu fixed-slice PPL from a local parquet snapshot."""
...
def flush_blocks(model, blocks, device, total_nll, total_tokens):
    ...
    return total_nll + float(loss.item()), total_tokens + labels.numel()
...
def main():
    ...
    data_files = sorted(glob.glob(args.data_files))
    if not data_files:
        raise FileNotFoundError(args.data_files)

    device = torch.device(args.device)
    start = time.time()
    metadata = {
        "model_path": args.model_path,
        "dataset": "local parquet HuggingFaceFW/fineweb-edu sample-10BT snapshot",
        "data_files": args.data_files,
        "num_data_files": len(data_files),
        "split": args.split,
        "note": "Fixed in-domain slice equivalent to sample-10BT train[-10000:] from local HF snapshot.",
    ...
    dataset = load_dataset("parquet", data_files=data_files, split=args.split)
