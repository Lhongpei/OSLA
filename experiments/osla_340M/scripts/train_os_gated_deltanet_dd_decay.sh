#!/bin/bash
# OS-GatedDeltaNet 340M with data-dependent d-decay on 8xH100.
# Same as train_os_gated_deltanet.sh but with decay_mode="data_dependent":
# a separate per-token per-head gate (from `osgm_a_proj → logsigmoid`) decays
# the OSGM preconditioner d_t between steps, mirroring the os_delta_net
# "osla_osgm_dd_decay" setup that produced the best OSGM extrapolation in
# the 340M summary.
#
# Shapes: head_dim=128, num_heads=6, expand_v=2, 21 layers → ~380M params.
